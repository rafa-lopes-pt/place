...
class AccordionItem extends Container {
    ...
    _containerToggleEventListener() {
        const header = this.instance?.find(`.${this.topClassBEM}__header`);
...
function _extractDigestInfo(res) {
    const payload = res;
    const info = payload?.GetContextWebInformation ?? payload?.d?.GetContextWebInformation;
    if (!info) return null;
...
function startDigestTimer() {
    ...
    if (typeof _spFormDigestRefreshInterval === "number" && _spFormDigestRefreshInterval > 0) {
        intervalMs = _spFormDigestRefreshInterval;
    } else if (typeof _spPageContextInfo !== "undefined" && typeof _spPageContextInfo.formDigestTimeoutSeconds === "number" && _spPageContextInfo.formDigestTimeoutSeconds > BUFFER_S) {
...
class UserIdentity {
    ...
    static fromField(value) {
        ...
        if (typeof value === "object" && !Array.isArray(value)) {
            const obj = value;
            try {
                return new UserIdentity(String(obj.email ?? ""), String(obj.displayName ?? ""));
            } catch {
                return null;
            }
        }
...
_UserIdentity_details = new WeakMap;
...
class CheckBox extends FormControl {
    ...
    _applyEventListeners() {
        super._applyEventListeners();
        this.setEventHandler("click", e => {
            if (!(e.target instanceof HTMLInputElement)) return;
            this._value.value = this.instance?.find("input")[0].checked ?? false;
            this._validate();
        });
    }
    toString() {
        return `<label \n                 id="${this.id}"\n                 class="${this.class} ${super.modifierClasses} " \n                 tabindex=0\n             >\n                 <input type="checkbox" ${this._value.value ? "checked" : ""}/>\n                 ${getIcon("check-line")}\n             </label>`;
    }
}
...
class Router {
    ...
    _addPopStateEventListeners() {
        window.addEventListener("popstate", async () => {
            if (__classPrivateFieldGet(this, _Router_navigationGuard, "f")) {
                ...
                if (typeof guardResult === "string") {
                    const shouldLeave = await this._showNavigationGuardDialog(guardResult);
                    if (!shouldLeave) {
                        const previousPath = __classPrivateFieldGet(this, _Router_lastPath, "f") ?? "/";
                        const hashPath = `#/${previousPath === "/" ? "" : previousPath}`;
                        history.pushState(null, "", hashPath);
...
class LinkButton extends Button {
    constructor(children, path, props) {
        const isExternal = LinkButton._isExternalUrl(path);
        super(children, {
            ...props,
