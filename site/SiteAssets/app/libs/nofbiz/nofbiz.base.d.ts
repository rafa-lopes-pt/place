 ...
 * `JSON.stringify()`, which invokes `toJSON()` producing `{ email, displayName }`.
 *
 * **Read:** ListApi auto-parses the JSON string back to a plain object --
 * `fromField()` accepts the resulting `{ email, displayName }` object.
 *
 ...
 * active.
 *
 * The interval is determined by (in priority order):
 * 1. `_spFormDigestRefreshInterval` (SharePoint global, pre-calculated ms)
 * 2. `_spPageContextInfo.formDigestTimeoutSeconds` minus a 2-minute buffer
 * 3. A 25-minute default fallback
 *
